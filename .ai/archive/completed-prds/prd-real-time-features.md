...
```bash
/ralph-loop:ralph-loop "Follow .ai/tasks/prd-real-time-features.md to add real-time zone timer. Invoke @implementation-planner for timer design, implement with React hooks, verify performance, commit. Output REAL_TIME_COMPLETE when done." --max-iterations 60 --completion-promise "REAL_TIME_COMPLETE"
```
