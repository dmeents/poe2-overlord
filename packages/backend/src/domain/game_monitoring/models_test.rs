...
mod tests {
    ...
    fn test_overlay_state_serialization() {
        ...
        assert!(json.contains("\"position\":[100,200]"));
        assert!(json.contains("\"size\":[800,600]"));
        assert!(json.contains("\"always_on_top\":true"));
    }

    #[test]
    fn test_overlay_state_deserialization() {
        let json = r#"{"visible":false,"position":[0,0],"size":[1920,1080],"always_on_top":false}"#;
        let state: OverlayState = serde_json::from_str(json).unwrap();
