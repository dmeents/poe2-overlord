...
impl LogFileRepository for LogFileRepositoryImpl {
    async fn get_file_info(&self, path: &str) -> AppResult<LogFileInfo> {
        ...
        Ok(LogFileInfo {
            ...
            last_modified: chrono::DateTime::<chrono::Utc>::from(metadata.modified().map_err(
                |e| AppError::file_system_error("Failed to get modified time: {}", &e.to_string()),
            )?),
        })
    }
    ...
    async fn read_lines(
        ...
        let file = fs::File::open(path)
            .await
            .map_err(|e| AppError::file_system_error("Failed to open file: {}", &e.to_string()))?;
        ...
        while let Some(line) = lines
            .next_line()
            .await
            .map_err(|e| AppError::file_system_error("Failed to read line: {}", &e.to_string()))?
        {
    ...
    async fn get_file_size(&self, path: &str) -> AppResult<u64> {
        let metadata = fs::metadata(path).await.map_err(|e| {
            AppError::file_system_error("Failed to get file size: {}", &e.to_string())
        })?;
        Ok(metadata.len())
    }
    ...
    async fn read_from_position(&self, path: &str, position: u64) -> AppResult<Vec<String>> {
        ...
        reader
            ...
            .await
            .map_err(|e| {
                AppError::file_system_error("Failed to seek to position: {}", &e.to_string())
            })?;
    ...
    async fn get_file_modified_time(&self, path: &str) -> AppResult<chrono::DateTime<chrono::Utc>> {
        ...
        Ok(chrono::DateTime::<chrono::Utc>::from(
            metadata.modified().map_err(|e| {
                AppError::file_system_error("Failed to get modified time: {}", &e.to_string())
            })?,
        ))
    }
}
