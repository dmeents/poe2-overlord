...
use app_lib::services::state_manager::StateManager;
use std::sync::Arc;
...
fn test_default_parser_config() {
    ...
    assert!(config.scene_change.patterns.contains(&"[SCENE] Set Source [".to_string()));
    assert!(config.scene_change.patterns.contains(&"[SCENE] Load Source [".to_string()));
