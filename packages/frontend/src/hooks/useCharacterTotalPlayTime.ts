...
export function useCharacterTotalPlayTime(characters: Character[]) {
  ...
  const fetchPlayTimeForCharacter = useCallback(async (characterId: string): Promise<number> => {
    try {
      const totalTime = await invoke<number>('get_character_total_play_time', {
        characterId,
      });
      return totalTime;
    } catch (err) {
      console.error(`Failed to fetch play time for character ${characterId}:`, err);
      return 0;
    }
  }, []);
  ...
  const getPlayTime = useCallback((characterId: string): number => {
